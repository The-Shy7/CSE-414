 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // use Dataset SparkSQL interface
        d.createOrReplaceTempView("flights");
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        Dataset<Row> r = spark.sql("SELECT origincityname AS origin_city_name, month, COUNT(*) AS count\n" +
                "FROM flights " +
                "WHERE cancelled != 1\n" +
                "GROUP BY origincityname, month");
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        Dataset<Row> r = spark.sql("SELECT origincityname AS origin_city_name, AVG(depdelayminutes) AS average\n" +
                "FROM flights " +
